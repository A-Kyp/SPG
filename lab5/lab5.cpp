...
void Lab5::Init()
{
    ...
    for (int i = 0; i < 40; ++i)
    {
        ...
        lightInfo.position = glm::vec3(rand_info1 * 10 * sign, rand_info2 * 3, rand_info3 * 10);
...
void Lab5::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_R) {
        ...
        for (auto& lightInfo : lights) {
            ...
            lightInfo.position = glm::vec3(rand_info1 * 10 * sign, rand_info2 * 3, rand_info3 * 10 * sign2);
            lightInfo.color = glm::vec3(rand_info1, rand_info2, rand_info3);
            lightInfo.radius = 3 + Rand01();
