...
void main()
{
    ...
    {
        // TODO(student): Rather than emitting vert`ices for the control

        float t;
        for(int i = 0; i < no_of_generated_points - 1; i++) {
            // t = 1.0f * i * 1/no_of_generated_points;
            // gl_Position = Projection * View * vec4(translateX(bezier(t), instance[0]), 1); EmitVertex();
            
            // t = 1.0f * i * 1/no_of_generated_points;
            // gl_Position = Projection * View * vec4(translateX(bezier(t), instance[0] + 1), 1); EmitVertex();
            
            // t = 1.0f * (i + 1) * 1/no_of_generated_points;
            // gl_Position = Projection * View * vec4(translateX(bezier(t), instance[0]), 1); EmitVertex();
            ...
            //Hermite 
            t = 1.0f * i * 1/no_of_generated_points;
            gl_Position = Projection * View * vec4(rotateY(hermite(t), instance[0]), 1); EmitVertex();
            
            t = 1.0f * i * 1/no_of_generated_points;
            gl_Position = Projection * View * vec4(rotateY(hermite(t), instance[0] + 1), 1); EmitVertex();
